...
export default function AddOffice({ onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-6">
      <div>
        <h2 className="text-xl font-semibold text-gray-800">Tambah Kantor Baru</h2>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <TextInput label="Latitude" value={position?.[0] || ""} readOnly />
        <TextInput label="Longitude" value={position?.[1] || ""} readOnly />
      </div>
      ...
      <div>
        <label className="block font-medium text-gray-700 mb-2">Pilih Lokasi di Peta</label>
        <div className="rounded-lg overflow-hidden shadow border border-gray-200 h-64">
          {isLoading ? (
            ...
            <div className="flex justify-center items-center h-full text-red-600 text-sm text-center px-4">
              Gagal mendapatkan lokasi perangkat. Pastikan izin lokasi aktif di browser Anda.
            </div>
          ) : (
            <MapContainer
              center={position}
